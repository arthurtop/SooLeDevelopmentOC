...
- (UIButton *)selectBtn{
    if (!_selectBtn) {
        ...
        _selectBtn.size = CGSizeMake(0, 0);
        [_selectBtn setBackgroundImage:[UIImage imageNamed:@""] forState:UIControlStateNormal];
...
- (UILabel *)titleLabel{
    if (!_titleLabel) {
        ...
        _titleLabel = [[UILabel alloc]initWithFrame:CGRectMake(self.selectBtn.right + 10, 0, width, 45)];
...
- (UIImageView *)iconImageView{
    if (!_iconImageView) {
        ...
        self.iconImageView.size = CGSizeMake(11, 11);
        self.iconImageView.right = kScreen_Width - 14;
    }
...
- (void)selectBtnClick:(UIButton *)btn{
    ...
    if ([self.delegate respondsToSelector:@selector(cell:didSelectedBtn:)]) {
        [self.delegate cell:self didSelectedBtn:btn];
    }
